...
temp_graph <- ggplot(sb_buoy_num, aes(x = atmp,
                                      y = month_name)) +
  geom_density_ridges(aes(color = month_name, fill = month_name, alpha = 0.5)) +
...
ggplot(data = mack_creek_weight_unit, aes(x = weight,
                                          y = unittype)) +
  geom_density_ridges(aes(color = unittype, fill = unittype, alpha = 0.7),
                      show.legend = FALSE) +
  scale_x_continuous(expand = c(0,0)) +
  theme_dark()
